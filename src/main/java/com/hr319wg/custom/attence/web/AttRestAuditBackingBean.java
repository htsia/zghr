...
public class AttRestAuditBackingBean extends BaseBackingBean {
	...
	public void queryAuditTask(){
		try{
			list=attBusiService.queryRestAuditTask(super.getUserInfo().getUserId());
		}catch(Exception e){
			e.printStackTrace();
		}
	}
	...
	public String saveAudit(){
		try {
			...
			this.attBusiService.saveRestAudit(result, bo, reason, taskId,
