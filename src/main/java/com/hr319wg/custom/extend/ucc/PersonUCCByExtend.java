...
public class PersonUCCByExtend extends PersonUCC {
	...
	public void chgDismiss(PersonChangeVO personchangevo,
			ChangeInfoBO changeinfo, String[] ids) throws RollbackableException {
		try {
			...
			if ((depID != null) && (!depID.equals(""))) {
				...
				if (orgx != null) {
					...
					for (int i = 0; i < ids.length; i++) {
			            ...
			            list.add("insert into A016(id,subid,A016010,A016020,A016030,A016040,A016045,A016050,A016200,A016201,A016202,A016203,A016204,A016205) values" +
			            		"('" + ids[i] + "','" + SequenceGenerator.getKeyId("A016") + "','" + PersonChangeVO.CHANGE_TYPE_IN + "','" + CommonFuns.filterNull(personchangevo.getChangeDate()) + "'," +
			            				"'" + CommonFuns.filterNull(superOrg.getName()) + "','" + CommonFuns.filterNull(personchangevo.getTractDate()) + "','" + CommonFuns.filterNull(personchangevo.getTractPerson()) + "','" + b730subid + "','"+po.getOrgId()+"','"+xt+"','"+po.getDeptId()+"','"+orgx.getOrgId()+"','"+po.getPostId()+"','"+postID+"')");
			            //工作简历子集
					...
					if ((list != null) && (list.size() > 0)) {
						this.getActivepageservice().batchExecuteSql(
								(String[]) (String[]) list
										.toArray(new String[0]));
					}

					String sql = "update A001 set A001701 = '" + xt
							+ "',A001705='" + depID + "',A001728='" + orgTreeid
							+ "',A001738='" + depTreeid + "',A001715='"
							+ postID + "',A001716='" + postLevel + "' where  "
							+ CommonFuns.splitInSql(ids, "ID");
					this.getActivepageservice().executeSql(sql);

					if (!"".equals(Constants.UUM_SERVICE_NAME))
						try {
							IUUManager uumanager = (IUUManager) SysContext
									.getBean(Constants.UUM_SERVICE_NAME);
							if (uumanager != null)
								for (int i = 0; i < ids.length; i++) {
			...
			throw new RollbackableException("E070001", "同步信息子集出错！"
