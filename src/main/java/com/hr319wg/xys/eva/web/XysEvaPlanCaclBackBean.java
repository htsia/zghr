...
public class XysEvaPlanCaclBackBean extends BaseBackingBean {
    ...
    public void useSpread() {
        try {
            ...
            XysEvaPlanBO bo = xysEvaPlanUCC.findXysEvaPlanBOById(planId);
            List glist = this.evaGradeUCC.getAllGradeItem(bo.getPlanGrade());
            ...
            if (setList != null && setList.size() > 0) {//强制分布
                if (glist != null && glist.size() > 0) {
                    for (int i = 0; i < glist.size(); i++) {
                        EvaGradeItemBO grade = (EvaGradeItemBO) glist.get(i);
        ...
        try {
            ...
            if (objList != null && objList.size() > 0) {
                for (int i = 0; i < objList.size(); i++) {
                    ...
                    double score = Double.parseDouble(CommonFuns.filterNullToZero(obj.getTotalScore()));
                    if (gradeList != null && gradeList.size() > 0) {
                        for (int j = 0; j < gradeList.size(); j++) {
    ...
    //导入部门考核分数
    private String initUploadDeptKpi;

    public String importDeptKpi() {
        try {
            String fileName = excelFile.getName();
            String type = fileName.substring(fileName.indexOf(".") + 1);
            if (type.equals("xls")) {
                ...
                for (int i = 1; i < rows; i++) {// 便利excel封装 KPIItemVO
                    ...
                    String score = CommonFuns.filterNull(cell[2].getContents().trim());
                    if (pbo != null) {
                        XysKpiDeptObjBO obj = this.xysKpiDeptUCC.findEqualsXysKpiDeptObjBO(planId, pbo.getPersonId());
                        if (obj == null) {
                            obj = new XysKpiDeptObjBO();
                        }
                        obj.setPersonId(pbo.getPersonId());
                        obj.setPlanId(planId);
                        obj.setDeptId(pbo.getDeptId());
                        obj.setScore(score);
                        this.xysKpiDeptUCC.saveXysKpiDeptObjBO(obj);
                    }
                }
                wb.close();
                XysEvaPlanBO bo = xysEvaPlanUCC.findXysEvaPlanBOById(planId);
    ...
    public String getInitUploadDeptKpi() {
        if (super.getRequestParameter("planId") != null) {
            planId = super.getRequestParameter("planId");
        }
        return initUploadDeptKpi;
    }

    public void setInitUploadDeptKpi(String initUploadDeptKpi) {
        this.initUploadDeptKpi = initUploadDeptKpi;
    }

    //导入个人KPI分数
    private String initUploadPersKpi;

     public String importPersKpi() {
        try {
            String fileName = excelFile.getName();
            String type = fileName.substring(fileName.indexOf(".") + 1);
            if (type.equals("xls")) {
                ...
                for (int i = 1; i < rows; i++) {// 便利excel封装 KPIItemVO
                    ...
                    String score = CommonFuns.filterNull(cell[2].getContents().trim());
                    if (pbo != null) {
                        XysKpiPersonObjBO obj = this.xysKpiPersonUCC.findEqualsXysKpiPersonObjBO(planId, pbo.getPersonId());
                        if (obj == null) {
                        ...
                        obj.setPlanId(planId);
                        obj.setScore(score);
                        this.xysKpiPersonUCC.saveXysKpiPersonObjBO(obj);
                    }
                }
                wb.close();
            } else {
    ...
    public String getInitUploadPersKpi() {
        if (super.getRequestParameter("planId") != null) {
            planId = super.getRequestParameter("planId");
        }
        return initUploadPersKpi;
    }

    public void setInitUploadPersKpi(String initUploadPersKpi) {
        this.initUploadPersKpi = initUploadPersKpi;
    }
}
