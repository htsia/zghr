...
public class PersonAddBackingBean extends BaseBackingBean {
	...
	public String createPerson() {
		try {
			...
			if (("1".equals(Constants.WAGE_POST_LINK)) && ("0".equals(Constants.EMP_ADD_AFTER_APPROVE) || Constants.EMP_ADD_AFTER_APPROVE == null)) {
			...
			if (this.type.equals("1")) {
				...
				String[] ValueList = { CommonFuns.filterNull(getEducation()), CommonFuns.filterNull(getGraduate()), CommonFuns.filterNull(getCollege()), CommonFuns.filterNull(getYear()) };

				this.personucc.addPageInfo("A004", user, "", personId, false, ColList, ValueList);

				ColList = new String[] { "A840701", "A840705" };
				ValueList = new String[] { CommonFuns.filterNull(getDegree()), CommonFuns.filterNull(getDegreeTime()) };

				this.personucc.addPageInfo("A840", user, "", personId, false, ColList, ValueList);
			} else if (this.type.equals("2")) {
				String[] ColList = { "A076701", "A076710", "A076715", "A076720" };
				String[] ValueList = { CommonFuns.filterNull(getReturnType()), CommonFuns.filterNull(getReturnTime()), CommonFuns.filterNull(getReturnDep()), CommonFuns.filterNull(getReturnDuty()) };
			...
			if ("1".equals(Constants.EMP_ADD_AFTER_APPROVE)) {
				...
				if ((this.personvo.getPersonIdentity() != null) && (Constants.EMP_CADRESCODE.indexOf(this.personvo.getPersonIdentity() + ",") >= 0)) {
				...
				WFDefineExcludeBO excludebo = this.wfservice.findWFDefineExcludeBO(WFTypeBO.RYGL_COMMON_IN);
				if ((excludebo == null) || (excludebo.getOrguid() == null) || ("".equals(excludebo.getOrguid())) || (excludebo.getOrguid().indexOf(org.getOrgId()) < 0)) {
			...
			if ((oo != null) && (oo.getWageItems() != null)) {
				...
				for (int j = 0; j < items.length; j++) {
					if (super.getRequestParameter(items[j]) != null) {
						...
						listmap.put(personId + "|" + items[j], super.getRequestParameter(items[j]));
					}
				}
				if (!"".equals(itemString)) {
					this.wagesetpersonucc.updateBaseInfo(pids, itemString.split(","), listmap);
				}
			}
			...
			String sql = null;
			String curr = personvo.getStatus();
	...
	public void importData(String personID) {
		try {
			...
			if (adjust == null) {
				...
				if ((opo != null) && (opo.getWageItems() != null) && (!opo.getWageItems().equals("")))
					this.adjustucc.createAdjustDetail(pb.getPersonId(), adjust.getItemID(), opo.getWageItems().split(","));
			}
	...
	public String createManger() {
		try {
			...
			if (ib != null)
				this.personvo.setPersonIdentity(ib.getItemDefaultValue());
