...
public class ATTWriterEvcctionBackingBean extends BaseBackingBean {
		...
		try{
			...
			if(list!=null && list.size()>0){
				for(int i=0;i<list.size();i++){
					String id=(String)list.get(i);
					AttEvcctionRecBO bo=new AttEvcctionRecBO();
	        		bo.setPersonID(id);
	    			...
	    			attevcctionreccollectbo.setCaclDate(attevcctionrecbo.getApplyDate());
	    			...
	    			attenceSetUCC.saveAttEvcctionRecCollectBO(attevcctionreccollectbo);
