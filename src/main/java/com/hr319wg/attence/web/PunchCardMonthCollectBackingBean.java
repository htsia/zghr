...
public class PunchCardMonthCollectBackingBean extends BaseBackingBean {
	private String orgID=super.getUserInfo().getOrgId();
	private IAttenceSetUCC attenceSetUCC;
	private String beginTime=new SimpleDateFormat("yyyy-MM").format(new java.util.Date());
	private String endTime=new SimpleDateFormat("yyyy-MM").format(new java.util.Date());
		...
		try {
			...
			if ("1".equals(backFlag))
				sql = attenceSetUCC.queryPersonListandmonth(table, name, "", orgID, 1, Constants.ACTIVE_PAGE_SIZE,
						Constants.NO, user,monthvalues,"",beginTime,endTime,userid);
			else
				sql = attenceSetUCC.queryPersonListandmonth(table, name,personType, orgID, 1, rowNum,
						"00900", user,monthvalues,"",beginTime,endTime,userid);
