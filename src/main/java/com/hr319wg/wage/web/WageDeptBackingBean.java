	...
	public List getPersonPowerList() {
		try {
			if (this.personID != null) {
				this.personPowerList = this.wagedeptucc.getAllPowerByPerson(
						this.personID, this.setID);
				for (int i = 0; i < this.personPowerList.size(); i++) {
					WageDeptPowerBO wdp = (WageDeptPowerBO) this.personPowerList
							.get(i);
					WageDeptBO wd = this.wagedeptucc.getDeptBOByID(wdp
							.getDeptID());
		...
		try {
			...
			for (int i = 0; i < this.list.size(); i++) {
				...
				if ((wb.getA001758() != null) && (!"".equals(wb.getA001758()))) {
					wb.setA001758(CodeUtil.interpertCode("2209",
							wb.getA001758()));
				}
				if (wb.getLinkOrgs() != null && !"".equals(wb.getLinkOrgs())) {
					wb.setLinkOrgNames(CodeUtil.interpertCode(
							CodeUtil.TYPE_ORG, wb.getLinkOrgs()));
				}
			}
			int size=this.list.size()+1;
			if(size<10){
				this.listSize="0"+size;
			}else{
				this.listSize=size+"";
			}
		} catch (SysException e) {
