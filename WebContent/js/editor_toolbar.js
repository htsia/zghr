...
function fGenToolBarHtml() {
	var commHeadMenu = [ '<div style="background:url(/images/editoricon.gif)  -$left$px 0;width:$w$px;margin:$m$;height:17px;float:left;" unselectable="on"' ].join( "" );
	var commTail = '></div>';
	...
	var templ = ( [ '<div id="$id$" style="background:url(/images/editoricon.gif) -$left$px 0;width:$w$px;margin:$m$;$s$" class="editor_btn" unselectable="on"  title="$title$" onmousedown=fOnTBMouseDown(this) onmouseover=fOnTBMouseOver(this) onmouseout=fOnTBMouseOut(this) onmouseup=fOnTBMouseOver(this)></div>' ].join( "" ) ).split("$");
	var code = [ 
	"<div id='", gEditorToolBarId, "' class='editor_toolbar' unselectable='on'>", 
		...
		"</div>", 
		"<div class='editor_abtn' id='editor_htmlbtn' onclick='fChangeEditor(1);' unselectable='on'  style='", partDisp[4], "' title='编辑HTML源码'>查看HTML代码</div>",
		"<div class='editor_abtn' id='editor_sourcebtn' style='display:none;' onclick='fChangeEditor();' unselectable='on' title='编辑'>编辑<b>&#187;</b></div>",
	"</div>" ];
