...
mmenucolor = '#2074C5'; // 菜单背景色
mfontcolor = '#FFFFFF'; // 字体颜色
mmenuoutcolor = '#FFFFFF';
mmenuincolor = '#FFFFFF';
mmenuoutbordercolor = '#2074C5';
mmenuinbordercolor = '#2074C5';
mmidoutcolor = '#A9DA1D';
mmidincolor = '#FF0000';
mmenuovercolor = '#000000';
mitemedge = '0';
msubedge = '1';
mmenuunitwidth = 100;
mmenuitemwidth = 170;
...
mmenuadjust = 0;
mmenuadjustV = 0;
mfonts = 'font-family: 宋体;font-size: 9pt;font-weight:thin;';
mcursor = 'default';
...
function mMenuItem(caption, command, target, isline, statustxt, level, img, sizex, sizey, pos) {
    ...
    this.level = level;
    this.img = img;
    if (sizex==null || sizex=="" ||sizex==undefined){
        this.sizex = "13";
    }
    else{
      this.sizex = sizex;
    ...
    if (sizey==null || sizey=="" ||sizey==undefined){
        this.sizey="13";
    }
    else{
       this.sizey = sizey;
...
function showObject(docu) {
    try {
        if (docu.frames.length > 0) {
            for (f = 0; f < docu.frames.length; f++) {
                doc = docu.frames(f).document;
                obj = doc.all.tags("SELECT");
                if (obj != null) {
...
// 输出到屏幕
//  div mposflag 用来取得位置坐标
function showMenu() {
    var mwb = 1;
    var stringx = '<div id="mposflag" style="position:absolute;"></div><table  id=mmenutable border=0 cellpadding=3 cellspacing=0 width=' + mmenuwidth + ' height=' + mmenuheight + ' bgcolor=' + mmenucolor +
    ...
    for (var x = 0; x < mmenus.length; x++) {
        ...
        if (thismenu.sizex != "0" && thismenu.sizey != "0") {
            ...
            imgsize = " width=13 height=13"; 
        }

        var ifspace = "";
        if (thismenu.caption != "") ifspace = "&nbsp;";
        stringx += "<td nowrap class=coolButton id=mMenu" + x + " style='border: " + mitemedge + "px solid " + mmenucolor + ";" + mfonts +
        ...
        } else if (thismenu.pos == "3" && thismenu.img!="" ) {
            stringx += " align=center><img align=absmiddle src='" + thismenu.img + "' "+imgsize+" >"+thismenu.caption+"</td>"; 
        } else {
        ...
        if (x < 0) {
            ...
            for (var i = 0; i < thismenu.items.length; i++) {
                ...
                if (thismenuitem.hasc != null) {
                    stringx += "<tr><td id=mp" + thismenuitem.hasc + " class=coolButton style='border: " + mitemedge + "px solid " + mmenucolor + ";" + mfonts + ";" +
                    ...
                    stringx += "<tr><td class=coolButton style='border: " + mitemedge + "px solid " + mmenucolor + ";" + mfonts + ";" +
    ...
    // 输出三级菜单
    var index=0;
    for (var j = 1; j <= allsub; j++) {
        thisitem = eval("msub" + j);
        if (thisitem.items==null || thisitem.items.length==0) continue;
        index++;
        stringx += '<div id=msubmenudiv' + index +
        ...
        for (var i = 0; i < thisitem.items.length; i++) {
            ...
            if (thismenuitem.hasc != null) {
                stringx += "<tr><td id=mp" + thismenuitem.hasc + " class=coolButton style='border: " + mitemedge + "px solid " + mmenucolor + ";" + mfonts + ";" +
                           "' width=100% onmouseout=mmenuitem_out(true) onmouseover=\"mmenuitem_over(msubmenudiv" + index +
                           ",'" + thismenuitem.hasc + "',-1," + j + "," + i + ");mshowsubmenu(msubmenudiv" + thismenuitem.hasc + ",mp" + thismenuitem.hasc + ",msubmenudiv" + index + ");\"" +
                           "><table id=mitem" + thismenuitem.hasc + " cellspacing='0' cellpadding='0' border='0' width='100%' style='" + mfonts + "'><tr><td ";
                if (thismenuitem.pos == "0") {
                    stringx += "><img align=absmiddle src='" + thismenuitem.img + "'" + imgsize + ">" + ifspace + thismenuitem.caption + "</td><td";
                } else if (thismenuitem.pos == "1") {
                ...
                stringx += "<tr><td class=coolButton style='border: " + mitemedge + "px solid " + mmenucolor + ";" + mfonts + ";" +
                           "' width=100% height=15px onmouseover=\"mmenuitem_over(msubmenudiv" + index +
