...
function getTreeImage() {
    try {
        var images = new tree_imagelist();
        images.path = "/images/tree_images/";
        images.type = "gif" ;
        images.add("fold2k", "folder") ;
        images.add("fold2k", "fold");
        images.add("org", "org") ;
        images.add("dept", "dept") ;
        images.add("group", "group")  ;
        images.add("virtual", "virtual") ;
        images.add("plus", "expand");
        images.add("minus", "collapse");
        images.add("hfile", "file") ;
        images.add("hfiles", "hfiles") ;
        images.add("person", "per") ;
        images.add("male", "male");
        images.add("female", "female");
        images.add("post", "post") ;
        images.add("book1_open", "book_open") ;
        images.add("book1_close", "book_close");
        images.add("find", "find");
        images.add("database", "database");
    } catch(e) {
        alert(e);
    }
    return images;
...
function tree_imagelist(){
    try{
        ...
        iml.err="imagelist出错,错误号:"+e.number+".\n错误描述:"+e.description;
        alert(iml.err);
        return iml;
    }
}

function newtree(imageList,indent,toObject,labelStyle){
    try{
	if(!document.getElementById){throw new Error(999,"需要IE6.0以上的浏览器!");}
	...
	var addNode=function(toNode,relation,text,key,key2,alt,ico,exeCategory,exeArg){
		var node={}
		node.key2=key2;
		node.alt=alt;
		...
		with(nbody){
            ...
            if (inLead==0){
                ...
                style.cssText="cursor:default;text-align:left;color:black;font-size:16px;margin:0;";
            }
		...
		node.label=label;
		node.icon=icon;
		node.add=function(text,key,key2,alt,ico,exeType,exeArg){return addNode(this,"last",text,key,key2,alt,ico,exeType,exeArg);}
	...
	tree.err="newtree cause run time error!\nError number:"+e.number+".\nError description:"+e.description;
	alert(tree.err);
	return tree;
}
}