...
%>
<x:saveState value="#{cont_contResumeBB}"/>
<html>
<head>
<title></title>
</head>
...
<h:form id="form1">
    <h:panelGrid width="100%" border="0" styleClass="td_title" cellspacing="0" cellpadding="0" >
       <h:panelGroup>
           ...
           <h:outputText value="恢复履行处理"  ></h:outputText>
        </h:panelGroup>
    </h:panelGrid>

    <h:inputHidden id="init_a" value="#{cont_contResumeBB.init_a}"/>
<h:inputHidden id="str" value="#{cont_contResumeBB.validDate}"/>
<h:panelGrid  border="0"  align="center" columns="1" width="95%">
     <h:panelGrid  border="0"  cellspacing="0" align="left" columns="13" >
         <h:outputText value="姓名："/>
         <h:inputText styleClass="input" size="15" id="personNameQry" value="#{cont_contResumeBB.personNameQry}"/>
        <h:commandButton styleClass="button01" value=" 查询 " action="#{cont_contResumeBB.queryAll}"/>
        <h:commandLink id="doQuery" action="#{cont_contResumeBB.queryAll}"></h:commandLink>
        <h:commandButton styleClass="button01" value="签订恢复履行协议" action="#{cont_contResumeBB.forwardAdd}" />
        <h:commandButton styleClass="button01" value="批量修改" action="#{cont_contResumeBB.findByIds}" onclick="return checkBatchDo('selected_ids');"/>
         <h:commandButton styleClass="button01" value="报审" action="#{cont_contResumeBB.toApprove}" onclick="if(checkBatchDo('selected_ids')) { return doApprove(); } else {return false;}"/>
         <h:commandButton styleClass="button01" value="撤销报审" action="#{cont_contResumeBB.toCancelApprove}" onclick="if(checkBatchDo('selected_ids')) { return doCancelApprove(); } else {return false;}"/>
        <h:commandButton styleClass="button01" value="主合同编号维护" action="#{cont_contResumeBB.editMainContCode}" onclick="return checkModiCode('selected_ids');"/>
        <h:commandButton styleClass="button01" value=" 生效" action="#{cont_contResumeBB.toValid}" onclick="javascript:return valid();"/>
        <h:commandButton styleClass="button01" value=" 删除" action="#{cont_contResumeBB.deleteBatch}" onclick="return checkBatchDelete('selected_ids');" />
    ...
    <h:panelGrid align="right" styleClass="locked_top">
        <h:panelGroup>
            <h:outputText value="记录数:#{cont_contResumeBB.pagevo.totalRecord}"></h:outputText>
            <h:outputText value="  "></h:outputText>
            <h:outputText value="页数:#{cont_contResumeBB.pagevo.totalPage}"></h:outputText>
            <h:outputText value="  "></h:outputText>
            <h:outputText value="每页有#{cont_contResumeBB.pagevo.pageSize}"></h:outputText>
            <h:outputText value="  "></h:outputText>
            <h:outputText value="当前为第#{cont_contResumeBB.pagevo.currentPage}页"></h:outputText>
            <h:commandButton value="首页" action="#{cont_contResumeBB.first}" styleClass="button01"></h:commandButton>
            <h:commandButton value="上页" action="#{cont_contResumeBB.pre}" styleClass="button01"></h:commandButton>
            <h:commandButton value="下页" action="#{cont_contResumeBB.next}" styleClass="button01"></h:commandButton>
            <h:commandButton value="尾页" action="#{cont_contResumeBB.last}" styleClass="button01"></h:commandButton>
        </h:panelGroup>
    </h:panelGrid>
    
</h:panelGrid>
<center>
<x:dataTable value="#{cont_contResumeBB.bos}" var="list"  rowIndexVar="index"
            ...
            <h:column>
                <c:facet name="header"><h:outputText value="恢复履行时间"/></c:facet>
                <h:outputText value="#{list.resumeExecuteContDate}"/>
            </h:column>
            ...
            <h:column>
                ...
                <h:outputText value="  "/>
                <h:commandLink id="bnt2" value="修改" action="#{cont_contResumeBB.findById}">
