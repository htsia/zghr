...
<script language="javascript">
  function selectNewPerson(ctrlobj){
	...
	retval = window.showModalDialog("/cont/SelectEndPerson.jsf", "", "dialogWidth:750px; dialogHeight:400px; dialogLeft:"+showx+"px; dialogTop:"+showy+"px; status:yes; directories:yes;scrollbars:yes;Resizable=yes; "  );
    if (retval != null) {
   ...
   function checkSubmit(form1) {
        ...
        if (form1.all("form1:generateDoc").checked){
            name= form1.all("form1:templateClassId").value;
            if (name == "" || name == null) {
                alert("模板类别不能为空！");
                 flag = false;
                 return flag;
...
<h:panelGrid width="100%" border="0" cellspacing="6" cellpadding="0"  styleClass="td_title">
   <h:panelGroup>
       ...
       <h:outputText value="合同续签维护"></h:outputText>
   </h:panelGroup>
</h:panelGrid>
...
<h:panelGrid  border="0"  align="center" columns="2" width="800">
   <%
    ...
    <h:panelGrid  border="0"  cellspacing="0" align="right" columns="3" >
        <h:panelGroup>
            <h:inputHidden id="ManyPerson" value="#{cont_contFollowSignBB.nameStrs}"></h:inputHidden>
            <h:commandButton value="查询终止人员" id="queryPerson2" onclick="return doQueryManyPerson()" styleClass="button01"
                             action="#{cont_contFollowSignBB.queryMultPerson}"/>
        </h:panelGroup>

         <h:commandButton styleClass="button01" value="选择到期人员" action="#{cont_contFollowSignBB.selectPerson}" onclick="javascript:return selectNewPerson();"/>
         <h:commandButton styleClass="button01" value="去除人员" action="#{cont_contFollowSignBB.delPerson}" onclick="javascript:return delPerson();"/>
    </h:panelGrid>
</h:panelGrid>
...
<h:panelGrid width="800" border="0" cellpadding="0" cellspacing="0" styleClass="table03" align="center" columns="4" columnClasses="td_form01,td_form02,td_form01,td_form02">
         ...
         <h:inputText styleClass="input" id="postName"  size = "30" maxlength="15" value="#{cont_contFollowSignBB.bo.postName}" alt="合同约定工作岗位|1|s" />
