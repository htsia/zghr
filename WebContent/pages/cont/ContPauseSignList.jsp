...
<h:form id="form1">
    ...
    <h:panelGrid width="100%" border="0" styleClass="td_title" cellspacing="0" cellpadding="0" >
        <h:panelGroup>
            ...
            <h:outputText value="合同暂停"  ></h:outputText>
        </h:panelGroup>
    </h:panelGrid>
    ...
    <h:panelGrid  border="0"  cellspacing="0" align="left" columns="13" >
        ...
        <h:commandButton styleClass="button01" value=" 查询 " action="#{cont_contPauseSignBB.queryAll}"/>
        <h:commandLink id="doQuery" action="#{cont_contPauseSignBB.queryAll}"></h:commandLink>
        <h:commandButton styleClass="button01" value="合同暂停" action="#{cont_contPauseSignBB.forwardAdd}" />
        <h:commandButton styleClass="button01" value="批量修改" action="#{cont_contPauseSignBB.findByIds}" onclick="return checkBatchDo('selected_ids');"/>
         <h:commandButton styleClass="button01" value="报审" action="#{cont_contPauseSignBB.toApprove}" onclick="if(checkBatchDo('selected_ids')) { return doApprove(); } else {return false;}"/>
         <h:commandButton styleClass="button01" value="撤销报审" action="#{cont_contPauseSignBB.toCancelApprove}" onclick="if(checkBatchDo('selected_ids')) { return doCancelApprove(); } else {return false;}"/>
        <h:commandButton styleClass="button01" value="合同编号维护" action="#{cont_contPauseSignBB.editMainContCode}" onclick="return checkModiCode('selected_ids');"/>
        <h:commandButton styleClass="button01" value=" 生效 " action="#{cont_contPauseSignBB.toValid}" onclick="javascript:return valid();"/>
        <h:commandButton styleClass="button01" value=" 删除 " action="#{cont_contPauseSignBB.deleteBatch}" onclick="return checkBatchDelete('selected_ids');" />
        ...
        <h:panelGrid align="right" styleClass="locked_top">
            <h:panelGroup>
                <h:outputText value="记录数:#{cont_contPauseSignBB.pagevo.totalRecord}"></h:outputText>
                <h:outputText value="  "></h:outputText>
                <h:outputText value="页数:#{cont_contPauseSignBB.pagevo.totalPage}"></h:outputText>
                <h:outputText value="  "></h:outputText>
                <h:outputText value="每页有#{cont_contPauseSignBB.pagevo.pageSize}"></h:outputText>
                <h:outputText value="  "></h:outputText>
                <h:outputText value="当前为第#{cont_contPauseSignBB.pagevo.currentPage}页"></h:outputText>
                <h:commandButton value="首页" action="#{cont_contPauseSignBB.first}" styleClass="button01"></h:commandButton>
                <h:commandButton value="上页" action="#{cont_contPauseSignBB.pre}" styleClass="button01"></h:commandButton>
                <h:commandButton value="下页" action="#{cont_contPauseSignBB.next}" styleClass="button01"></h:commandButton>
                <h:commandButton value="尾页" action="#{cont_contPauseSignBB.last}" styleClass="button01"></h:commandButton>
            </h:panelGroup>
        </h:panelGrid>
...
<x:dataTable value="#{cont_contPauseSignBB.bos}" var="list"  rowIndexVar="index"
            ...
            <h:column>
                <c:facet name="header"><h:outputText value="中止合同约定开始时间"/></c:facet>
                <h:outputText value="#{list.contStartDate}"/>
            </h:column>
            <h:column>
                <c:facet name="header"><h:outputText value="中止合同约定结束时间"/></c:facet>
            ...
            <h:column>
                ...
                <h:outputText value="  "/>
                <h:commandLink id="bnt2" value="修改" action="#{cont_contPauseSignBB.findById}">
