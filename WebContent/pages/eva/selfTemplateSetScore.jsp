...
%>
   <%
       ...
       EvaObjectsUCC  objUcc=null;
       EvaPlanUCC  planUcc=null;
       String score="";
       try {
           ucc = (EvaKeyItemUCC) SysContext.getBean("eva_keyitemucc");
           objUcc=(EvaObjectsUCC)SysContext.getBean("eva_objectUCC");
           planUcc=(EvaPlanUCC)SysContext.getBean("eva_planUCC");
           if(masterID!=null&&!masterID.equals("")){
        	   score=objUcc.findEvaMasterBOById(masterID).getScore();
        	   String nextScore=(String)session.getAttribute("nextscore");
        	   if(nextScore!=null){
        		   score="";
        		   session.removeAttribute("nextscore");
        	   }
    	   }
       }
       catch (Exception e) {
...
<h:form id="form1">
    ...
    <h:commandButton id="savebutton" value="保存" style="display:none;" action="#{eva_scoreBB.savescore}"></h:commandButton>
    <c:verbatim>
   ...
   <c:verbatim>
               ...
               &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <%
                	...
                	if(score!=null&&!score.equals("")){
                		out.println("总分："+score+"&nbsp;&nbsp;&nbsp;");
                	}
                    if (masterID!=null && !"".equals(masterID) && objectID!=null && !"".equals(objectID)) {
                        out.println("<input type=\"button\" class=\"button01\" value=\"保存\" onclick=\"checkAllInput()\">");
                    }
                %>
            </td>
            <td>
       ...
       <tr><td valign="top" width="60%" algin=right>
                       ...
                       if (items!=null && items.size()>0){
                           ...
                           for (int j = 1; j < items.size(); j++) {
                               ...
                               if(keybo.getItemType().equals("2")){
                            	   ...
                            	   out.println("<td class='td_middle_left'>");
    	                           out.println("<input id='socre_"+keybo.getItemID()+"' name='"+id+"' class='input' type='text' onfocus=\"Info('"+keybo.getItemID()+"')\" value='"+value+"' onchange=\"checkInputValue('"+keybo.getItemID()+"')\" style=\"behavior:url('/js/mytextbox.htc');\" >");
    	                           out.println("</td>");
                               }
                               out.println("</tr>");
                           }
       ...
       <td valign="top"   align="left" width="40%" rowspan="2">
