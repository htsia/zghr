...
<%
    ...
    if (curCode != null && !"".equals(curCode)) {
        ...
        for (int i = 0; i < codeArrray.length; i++) {
            PartyBO party = SysCacheTool.findParty(codeArrray[i]);
            if (party != null)
                trees += ",'" + party.getTreeId() + "'";
        }
...
function showTree(pid, rightFlag, rootId, partyType) {
    ...
    xmlHttp.open("GET", "/pages/ajax/Party.jsp?superId=" + superId + "&rightFlag=" + rightFlag + "&rootId=" + rootId + "&paType=" + partyType, true);
    xmlHttp.send(null);
...
<div width="100">
    <input type="button" value="选择" onclick="doSel()" class="buttonface01">
    <input type="button" value="清除" onclick="doClear()" class="buttonface01"></div>
