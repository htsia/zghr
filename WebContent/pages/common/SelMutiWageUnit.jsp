...
<%
    ...
    if (curCode != null && !"".equals(curCode)) {
        ...
        for (int i = 0; i < codeArrray.length; i++) {
            WageUnitBO org = SysCacheTool.findWageUnit(codeArrray[i]);
            if (org != null)
                trees += ",'" + org.getTreeId() + "'";
        }
...
function showTree(pid, rightFlag, rootId) {
    ...
    xmlHttp.open("GET", "/pages/ajax/WageUnit.jsp?superId=" + superId + "&rightFlag=" + rightFlag + "&rootId=" + rootId, true);
    xmlHttp.send(null);
...
<div width="100">
    ...
    <input type="button" value="清除" onclick="doClear()" class="button01"></div>
