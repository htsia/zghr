...
<script type="text/javascript">
	function doViewAtt(qid, code) {
		window.open('/system/survey/ShowAtt.jsf?QuestionID=' + qid + '&Code='
				+ code, "_blank", "top=" + screen.height * 0.1 + ",height="
				+ screen.height * 0.8 + ", left=" + screen.width * 0.1
				+ ",width=" + screen.width * 0.8
				+ ",toolbar=no,scrollbars=auto,menubar=no");
		return false;
	}
	function showInput() {
		window.showModalDialog("/system/survey/ShowInput.jsf?paperID="
				+ document.all('form1:paperID').value, null, "dialogWidth:"
				+ screen.width * 0.8 + "px; dialogHeight:" + screen.height
				+ 0.7 + "px;center:center;resizable:yes;status:no;scroll:yes;");
		return false;
	}
	function OpenRpt() {
		...
		var url = "/pages/report/ServerReportShow.jsp?RptCode="
				+ document.all("form1:ReportID").value + "&ID="
				+ document.all('form1:paperID').value;
		window.open(url);
		return false;
	}
</script>
...
<h:form id="form1">
	<h:inputHidden id="pageInit" value="#{self_surveyprocessBB.showResult}" />
	<h:inputHidden id="paperID" value="#{self_surveyprocessBB.paperID}"></h:inputHidden>
	<c:verbatim>
		<table height=98% width=98% align="center">
			<tr>
				<td align="right" height=8px></c:verbatim> <h:selectOneMenu
					...
					</h:selectOneMenu> <h:selectBooleanCheckbox value="#{self_surveyprocessBB.percent}"></h:selectBooleanCheckbox>

					<f:verbatim>
						<%=LanguageSupport
							.getResource("XTGL-1377", "显示百分比 ")%>:
               	 <%=LanguageSupport.getResource("XTGL-1378", "回收问卷")%>:
	          </f:verbatim> <h:outputText value="#{self_surveyprocessBB.receivedPaper}"></h:outputText>
					<h:commandButton styleClass="button01" value="刷新" id="refresh"
