...
<x:saveState value="#{ins_BaseOrgInfoBB}" />

<h:form id="form1">
	<h:inputHidden value="#{ins_BaseOrgInfoBB.pageInit}" />
	<h:inputHidden id="unitId" value="#{ins_BaseOrgInfoBB.unitId}" />
	<h:inputHidden id="str" value="" />
	<h:inputHidden id="deptpower" value="#{ins_BaseOrgInfoBB.deptpower}" />
	<f:verbatim>
		<table width=98% height=100% border=0 align="center">
			<tr>
				<td height=8px></f:verbatim>
              <h:panelGrid styleClass="td_title" width="100%"
                ...
                <h:panelGrid columns="5" align="center" width="98%">
					<h:panelGroup>
						<h:outputText escape="false"
							value="<strong>当前单位：</strong>#{ins_BaseOrgInfoBB.unitName}" />
					...
					<h:panelGrid align="right">
						<h:panelGroup>
                            <h:inputHidden id="calcSetString" value="#{ins_BaseOrgInfoBB.calcSetString}"></h:inputHidden>
                            <h:commandButton value="通过基数计算缴费金额" onclick="return selectBaseSet();"
								action="#{ins_BaseOrgInfoBB.calc}" styleClass="button01" />
					...
					<h:panelGroup>
						<h:selectOneMenu id="ReportID" style="width:150px">
							<c:selectItems value="#{ins_BaseOrgInfoBB.reportList}" />
						</h:selectOneMenu>
					...
					<h:panelGrid columns="5" align="right">
						...
						<h:commandButton id="query" value="查询"
							action="#{ins_BaseOrgInfoBB.queryPersonByNameStr}"
							styleClass="button01" />
						<h:commandButton value="高级查询" styleClass="button01"
							action="#{ins_BaseOrgInfoBB.queryAdvance}"
							onclick="return doAdvanceQuery('A','ORG','011','Y','','','N','152','N')" />
					</h:panelGrid>

					<h:panelGroup>
						<h:commandButton value="基数测算"
							action="#{ins_BaseOrgInfoBB.queryInsuranceBySetId}"
							onclick="doCountBase();" rendered="#{ins_BaseOrgInfoBB.operApply}" styleClass="button01" />
						...
						<h:selectOneMenu id="inputField"
							value="#{ins_BaseOrgInfoBB.inputField}">
							<c:selectItems value="#{ins_BaseOrgInfoBB.inputList}"></c:selectItems>
						</h:selectOneMenu>
						<h:commandButton value="修改"
							action="#{ins_BaseOrgInfoBB.queryInsuranceBySetId}"
							styleClass="button01" onclick="return inputBaseRate();"></h:commandButton>
						<h:commandButton value="录入新基数" onclick="return doSelectInfoInput();"  styleClass="button01" action="#{ins_BaseOrgInfoBB.queryPersonByNameStr}"/>
