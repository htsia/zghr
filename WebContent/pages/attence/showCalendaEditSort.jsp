...
<%@ page import="com.hr319wg.attence.pojo.bo.AttWorkCalendarBO" %>
...
<%
    ...
    try {
        ...
        list = rc.findAttWorkCalendarBO1(personID,aDate);
    }
    catch (Exception e) {
...
<h:form id="form1">
    <h:inputHidden value="#{groupCalendarEditBB.pageInits}"/>
<c:verbatim>
...
<c:verbatim>
                ...
                <select name="showItem" size="28" multiple id="showItem" style="width:100%;">
                    <%
                        for (int i = 0; i < list.size(); i++) {
                        	 AttWorkCalendarBO bo=(AttWorkCalendarBO)list.get(i);
                    %>
                    <option value="<%=bo.getItemID()%>"><%=bo.getActivity()%></option>
                    <%
                        }
                    %>
                </select>
        ...
        <tr>
            ...
            <td class="td_top"   align="right">
              ...
              <h:commandButton value="保  存"   styleClass="button01"    onclick="return forSave();"   action="#{groupCalendarEditBB.saveSort}"/>
<c:verbatim>
            </td>
        </tr>
</c:verbatim>
