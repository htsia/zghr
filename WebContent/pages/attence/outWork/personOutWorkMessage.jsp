...
function selectNewPersonTimeOff(){
	...
	if (perIds.length <= 0) {
		...
		window.showModalDialog("/attence/outWork/personOutWorkMessageTimeOff.jsf?personIdTimeOff="+perIds+"&personIdTimeOffDay="+personIdTimeOffDay, null, "dialogWidth:"+screen.width*0.6+"px; dialogHeight:"+screen.height*0.6+"px;center:center;resizable:yes;status:no;scroll:yes;");
	}
	return true;
}
</script>
...
<h:form id="form1">
	<h:commandButton id="refresh" value="刷新" style="display:none;" action="#{personoutworkBB.makesure}"></h:commandButton>
    <h:inputHidden value="#{personoutworkBB.pageInit1}" />
	<c:verbatim>
		<input type="hidden" name="content" value="">
	</c:verbatim>
	<h:panelGrid width="98%" align="center">
		<h:outputText escape="false" value="公出申请单"
		...
		<table align="center" border="0" width="610" id="table1"
			...
			<h:outputText escape="false" style="text-align:center;width:100%"
				...
				<td class="table_rlbt" align="center" colspan="2" width=200>
					...
					value="#{personoutworkBB.bo.dest}" />
					<f:verbatim>
				</td>
			</tr>

			<tr>
				<td align="center" height="40" class="table_rlbt">公出类型</td>
				<td height="40" colspan=6 class="table_rlbt">
				</f:verbatim>
			<h:selectOneRadio value="#{personoutworkBB.setStatus}" valueChangeListener="#{personoutworkBB.changeSetStatus}" disabled="#{personoutworkBB.bo.superFlId=='2' || personoutworkBB.bo.superFlId=='3'}"
				onclick="submit();">
...
<f:verbatim>
			<tr>
				<td align="center" height="40" class="table_rlbt">公出人员</td>
				<td height="40" colspan=6 class="table_rlbt">
				</f:verbatim>
				<x:dataTable value="#{personoutworkBB.listscommon}" var="list"
			...
			<h:commandButton styleClass="button01" value="选择人员" rendered="#{personoutworkBB.setStatus=='2'}"
				onclick="return selectNewPerson();" />
			<h:commandButton styleClass="button01" value="去除" rendered="#{personoutworkBB.setStatus=='2'}"
				action="#{personoutworkBB.remove}"
				onclick="javascript:return deleteNewPerson();" />
			<h:commandButton styleClass="button01" value="倒休设置"  onclick="selectNewPersonTimeOff();" rendered="#{personoutworkBB.personTimeOffState=='1'}"/>
				<f:verbatim>
				</td>
			</tr>
</f:verbatim>
...
<f:verbatim>
			...
			<tr>
				<td align="center" height="40" class="table_rlbt">公出时间</td>
				<td height="40" colspan=6 class="table_rlbt">
				</f:verbatim>
			<h:panelGroup>
			...
			<h:inputText styleClass="input" style="border:0px;" id="totalday" 
				value="#{personoutworkBB.kqglLeaverDayValue}" readonly="true"/>
				<f:verbatim>
				</td>
			</tr>
...
<h:inputTextarea id="desc" value="#{personoutworkBB.bo.reason}"
...
<h:panelGroup rendered="#{personoutworkBB.personTimeOffState=='1'}">
<f:verbatim>			
		    <tr>
			...
			<h:inputText styleClass="input" style="border:0px;" id="totalday1" 
				value="#{personoutworkBB.personTimeOffDay}" readonly="true"/>
				<f:verbatim>
				</td>
			</tr>
	        ...
	        <tr>
				...
				<td height="40" colspan=6 class="table_rlbt">
                  ...
                  <x:dataTable value="#{personoutworkBB.personIdTimeOffList}" var="list"
...
<f:verbatim>
            ...
            <h:commandButton styleClass="button01" value="保存" action="#{personoutworkBB.personEvectionMessage}"
                             onclick="return check_test();"  />
            <h:outputText value="  " />
            <h:outputText value="  " />
            <h:outputText value="  " />
            <h:commandButton styleClass="button01" value="确认并报批" action="#{personoutworkBB.personEvectionMessageAndPersonEvectionMessageAudit}"
                             onclick="return check_test();"  />
				<f:verbatim>
				</td>
			</tr>
