...
%>
    <script type="text/javascript">
        ...
        function doDetailPopup(id) {
            var dateId = id;
            popup = window.open("/attence/AttDetailView.jsf?dateId=" + dateId, "员工工资发放明细", "height=600,width=800,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
            popup.focus();
        }
...
<h:form id="form1">
    ...
    <h:panelGrid width="98%" align="center"  columnClasses="td_title" bgcolor="#FFFFFF" cellpadding="0" cellspacing="0">
        <h:panelGroup>
            ...
            <h:outputText value=" 考勤管理 ->  考勤信息处理"/>
        </h:panelGroup>
    </h:panelGrid>
    ...
    <h:dataTable value="#{wage_dateBB.dateList}" var="list" align="center"    id="dateList"
        ...
        <h:column>
            ...
            <h:panelGrid columns="5" align="center" cellspacing="2">
                <h:commandButton value="打开" action="#{att_computelistBB.queryInsuranceBySetId}" rendered="#{list.status=='新建' || list.status=='退回'}"
                                 styleClass="button01" >
                    <x:updateActionListener property="#{att_computelistBB.dateId}" value="#{list.dateId}"/>
                    <x:updateActionListener property="#{att_computelistBB.payoffDate}" value="#{list.date}"/>
                    <x:updateActionListener property="#{att_computelistBB.setId}" value="#{wage_dateBB.setId}"/>
                    <x:updateActionListener property="#{att_computelistBB.unitId}" value="#{wage_dateBB.unitId}"/>
                    <x:updateActionListener property="#{att_computelistBB.setName}" value="#{wage_dateBB.setName}"/>
                    <x:updateActionListener property="#{att_computelistBB.unitName}" value="#{wage_dateBB.unitName}"/>
                    <x:updateActionListener property="#{att_computelistBB.operCompute}" value="#{list.operCompute}"/>
                </h:commandButton>
                <h:commandButton value="查看明细" styleClass="button01" rendered="#{list.status!='新建' && list.status!='退回'}"
