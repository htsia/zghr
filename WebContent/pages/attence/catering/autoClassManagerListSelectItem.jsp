...
<x:saveState value="#{autoclassmanagerlistselectitemBB}" />
<h:form id="form1">
	<h:inputHidden id="pageInit"
		value="#{autoclassmanagerlistselectitemBB.pageInit}" />
	<h:inputHidden id="itemId"
		value="#{autoclassmanagerlistselectitemBB.itemId}" />
	<h:panelGrid styleClass="td_title" width="100%" border="0"
		cellpadding="0" cellspacing="6" bgcolor="#FFFFFF" columns="2">
		<h:panelGroup>
			<h:graphicImage value="/images/tips.gif" />
			<h:outputText value="自动排班计划管理人员调整" />
		</h:panelGroup>
				<h:panelGroup>>
						<h:graphicImage value="/images/attence/clock.gif" />
			<h:outputText value="#{autoclassmanagerlistselectitemBB.createDay}" />
		</h:panelGroup>
	</h:panelGrid>
	<c:verbatim>
		<br>
		...
		<table id="ryTable" width="800" border="0" align="center">
			<tr>
				<td></c:verbatim> <x:dataTable
					value="#{autoclassmanagerlistselectitemBB.personArrangeList}"
	...
	<h:panelGrid columns="4" align="center" width="95%"
		...
		<h:selectManyListbox size="15" style="width:250px" id="preTreat">
			<c:selectItems value="#{autoclassmanagerlistselectitemBB.workList}" />
		</h:selectManyListbox>

		<h:panelGroup>
			<h:commandButton styleClass="button01"
				onclick="Add(form1.all('form1:preTreat'))" value=">>"
				action="#{autoclassmanagerlistselectitemBB.add}"
				rendered="#{autoclassmanagerlistselectitemBB.isExist==true}" />
			<f:verbatim>
				<br>
			</f:verbatim>
			<h:commandButton styleClass="button01"
				onclick="Add(form1.all('form1:rsItem'))" value="<<" action="
				#{autoclassmanagerlistselectitemBB.delete}" rendered="#{autoclassmanagerlistselectitemBB.isExist==true}" />
		</h:panelGroup>

		<h:selectManyListbox size="15" style="width:250px" id="rsItem">
			<c:selectItems value="#{autoclassmanagerlistselectitemBB.restList}" />
		</h:selectManyListbox>
