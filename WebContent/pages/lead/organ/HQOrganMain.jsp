...
<h:form id="form1">
    ...
    <c:verbatim>
        ...
        <tr>
            ...
            <td align="right" >
                <font color="white"><%=user.getName()%>，您好！ 欢迎登录<% out.println(Constants.SYSTEM_NAME+Constants.LEAD_SYSTEM_NAME);%><br>上次登录：<%=user.getLastLoginTime()%>这是您今天第<%=user.getTodayTimes()%>次登录本系统</font>
            </td>
       </tr>
      </table>
    </c:verbatim>
  ...
  <tr>
    <td height="22" bgcolor="<%=Constants.MENU_COLOR%>">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td bgcolor="<%=Constants.MENU_COLOR%>" width="<%if(menu!=null) if(menu.getMenus()!=null) out.print(menu.getMenus().size()*25);%>"><script type="text/javascript" language="JavaScript1.2">
                        <%
                           if(menu != null){
                                subMenuCount=0;
