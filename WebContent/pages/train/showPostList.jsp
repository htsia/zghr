...
<%
    ...
    if (!"".equals(CommonFuns.filterNull(personId))) {
        PostBO post = SysCacheTool.findPost(personId);
        if (post != null) {
...
function showTree(pid, rightFlag, rootId) {
    ...
    xmlHttp.open("GET", "/pages/ajax/PostTree.jsp?superId=" + superId + "&rightFlag=1&rootId=" + rootId + "&filterSQL=<%=CommonFuns.filterNull(request.getParameter("filterSQL"))%>" , true);
    xmlHttp.send(null);
}
...
function parseResults() {
    ...
    var persons = results.getElementsByTagName("post");
    if (superId == "-1") {
    ...
    tree.onclick = function(srcNode) {
    	if(srcNode.alt == 'post')
	       parent.refreshList(srcNode.key2);
            ...
            parent.scale = srcNode.getKey().substring(1);             
            parent.scaleName.innerHTML = "在" + srcNode.label.innerText + "范围内";
        }
    }
</script>
