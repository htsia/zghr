...
<!-- 		其它1 -->
		<div class="big1">一、其它1(共<%=service.getWageOtherItemSum("1", wageDate, wageSetID) %>元)</div>
		<%
			...
			if(setList!=null){
				...
				for(WageDataSetBO setBo : setList){
					%>
					<div class='big2'><%=(i++)+"、"+setBo.getName() %>共<%=setBo.getTotalmoney() %>元</div>
					<table class="table03" border="<%=!"1".equals(p1)?"1":"0"%>">
						<tr>
							<th class="td_top">人员编号</th><th class="td_top">姓名</th><th class="td_top">所在部门</th><th class="td_top">金额</th><th class="td_top">备注</th>
						</tr>
						<%
		...
		%>
<!-- 		其它2 -->
		<div class="big1">二、其它2(共<%=service.getWageOtherItemSum("3", wageDate, wageSetID) %>元)</div>
		<%
			...
			if(setList!=null){
				...
				for(WageDataSetBO setBo : setList2){
					...
					<table class="table03" border="<%=!"1".equals(p1)?"1":"0"%>">
						...
						<%
							List<WageDataRecordBO> recList = service.getWageOtherItemDetail("3", setBo.getID(), wageDate, null, wageSetID);
		...
		%>
		<!-- 		扣其它1 -->
		<div class="big1">三、扣其它1(共<%=service.getWageOtherItemSum("4", wageDate, wageSetID) %>元)</div>
		<%
		...
		%>
		<!-- 		扣其它2 -->
		<div class="big1">四、扣其它2(共<%=service.getWageOtherItemSum("5", wageDate, wageSetID) %>元)</div>
		<%
...
<!-- 		捐款 -->
		...
		<%
			List<WageDataSetBO> setList3 = service.getAllWageSetBO("2", wageDate);
			if(setList!=null){
				int i=1;
				for(WageDataSetBO setBo : setList3){
					%>
					<div class='big2'><%=(i++)+"、"+setBo.getName() %>共<%=setBo.getTotalmoney() %>元</div>
					<table class="table03" border="<%=!"1".equals(p1)?"1":"0"%>">
						<tr>
						...
						<%
							List<WageDataRecordBO> recList = service.getWageOtherItemDetail("2", setBo.getID(), wageDate, null, wageSetID);
