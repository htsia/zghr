...
function checkSubmit() {
	...
	function selMonth() {
		if (confirm("确定要计入工资吗？这将清空本年度的所有存休记录")) {
			var month = window
					.showModalDialog(
							"/pages/common/CalendarMonthOnly.jsp",
							"选择日期",
							"dialogWidth:250px; dialogHeight:100px;center:center;resizable:yes;status:no;scroll:yes;");
			if (month != null) {
	...
	function modify(id) {
		window
				.showModalDialog(
						"/custom/attence/attPutoffMgrResultEdit.jsf?subID="
								+ id,
						null,
						"dialogWidth:400px;dialogHeight:500px;center:center;resizable:no;status:no;scroll:yes;");
		return true;
	}
	...
	function modify1() {
		window
				.showModalDialog("/custom/attence/attPutoffMgrResetEdit.jsf",
						null,
						"dialogWidth:350px; dialogHeight:230px; status:0;resizable:yes");
		return true;
	}
	function yearAttenceDay() {
		window
				.showModalDialog(
						"/custom/attence/attPutoffMgrYearAttenceEdit.jsf",
						null,
						"dialogWidth:350px; dialogHeight:230px; status:0;resizable:yes");
		return true;
	}
...
<h:form id="form1"  enctype="multipart/form-data">
	...
	<h:inputHidden id="personTypeValue"
		value="#{attPutoffMgrBB.personTypeValue}" />
	<c:verbatim>
		<table height=100% width=100% align="center">
			<tr>
				<td height=1></c:verbatim> <h:panelGrid width="98%" columns="1">
						<h:panelGrid align="left" columns="1">
							<h:panelGroup>
								<h:commandButton value="重置带薪假天数" onclick="return modify1()"
									styleClass="button01"></h:commandButton>
								<h:outputText value="  " />
								<h:outputText value="姓名或编号" />
								<h:inputText value="#{attPutoffMgrBB.nameStr}"
									styleClass="input" size="10" />
								<h:commandButton styleClass="button01" value="人员类别"
									onclick="return forSel();" action="#{attPutoffMgrBB.doQuery}" />
								<h:commandButton styleClass="button01" value="查询"
									action="#{attPutoffMgrBB.doQuery}" />
								<f:verbatim>
									<span align="right">
								</f:verbatim>
								<h:outputText value="选择发放工资的月份" />
								<h:inputText id="selectMonth" readonly="true" styleClass="input"
									size="12"
									onclick="WdatePicker({alwaysUseStartDate:true,dateFmt:'yyyy-MM'})"
								...
								<h:commandButton value="首页" action="#{attPutoffMgrBB.first}"
									styleClass="button01"></h:commandButton>
								<h:commandButton value="上页" action="#{attPutoffMgrBB.pre}"
									styleClass="button01"></h:commandButton>
								<h:commandButton value="下页" action="#{attPutoffMgrBB.next}"
									styleClass="button01"></h:commandButton>
								<h:commandButton value="尾页" action="#{attPutoffMgrBB.last}"
									styleClass="button01"></h:commandButton>
							</h:panelGroup>
						</h:panelGrid>
			...
			<tr>
				<td>
					<div style='width: 100%; height: 100%; overflow: auto' id=datatable>
						...
						<h:dataTable value="#{attPutoffMgrBB.list}" var="list"
							...
							<h:column>
								...
								<h:commandButton value="修改" styleClass="button01"
									onclick="modify('#{list.id}');"></h:commandButton>
							</h:column>
							<h:column>
								<c:facet name="header">
									<h:outputText value="操作" />
