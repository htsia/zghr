...
%>
    ...
    <script type="text/javascript" contenteditable="inherit">
    	function add() {
            window.showModalDialog("/custom/attence/attOutInputEdit.jsf?act=init", null, "dialogWidth:"+screen.width*0.5+"px;dialogHeight:"+screen.height*0.5+"px;center:center;resizable:no;status:no;scroll:yes;");
            return true;
       }
       function setData(v){
			if(v==0){
...
<h:inputHidden value="#{attOutApplyBB.inputInit}"/>
<h:form id="form1">
    ...
    <h:dataTable value="#{attOutApplyBB.list}" var="list" align="center" id="dateList"
        ...
        <h:column>
            ...
            <h:commandButton value="删除"  styleClass="button01" onclick="return confirm('确定删除吗');" action="#{attOutApplyBB.deleteInput}">
            	<x:updateActionListener value="#{list.id}" property="#{attOutApplyBB.operItemID}"></x:updateActionListener>
            </h:commandButton>
        </h:column>
    </h:dataTable>
    <c:verbatim>
