function createOperator(dataType) {
    ...
    operatorHTML = "<select name='operId' onchange='operatorChange(this)'>" +
                   ...
                   "<option value='isnull'>	为空	</option>" +
                   "<option value='notisnull'>	不为空	</option>" +
                   "<option value='allvalue'>	取全值	</option>";
    return operatorHTML;
}
    ...
    function modifyCondition(itemId, itemName, itemType, codeSetId) {
        ...
        //判断 是否要加选择按钮
        if (",5,6,7,8,11,12,".indexOf(","+itemType+",") != -1) {
    ...
    function insertConditionRow(itemId, itemName, itemType, codeSetId, operator, hidden1, value1, hidden2, value2) {
        ...
        c5.innerHTML = "<a onclick='deleteCondition(" + newRow.id + ")'>删</a>&nbsp;&nbsp;"
        c5.innerHTML += "<a onclick='moveUp(" + newRow.id + ")'>上</a>&nbsp;&nbsp;"
        c5.innerHTML += "<a onclick='moveDown(" + newRow.id + ")'>下</a>&nbsp;&nbsp;"
        //draw spanGroup
        drawGroup(conditionId)
    ...
    function doPop(src) {
        ...
        if ("5" == item_type) {//日期
            ...
            if (mutilSelFlag)
                PopUpMutilOrgTwoControl(codec, valuec);
            else
                fPopUpOrgDlg2(codec, valuec,'1');
        } else if ("8" == item_type) {//person
            if (mutilSelFlag)
                fPopUpMutiPerDlg(codec, valuec,'1');
            else
    ...
    function operatorChange(src, saveData) {
        ...
        if (src.value == "isnull" || src.value == 'notisnull' || src.value == 'allvalue') {
