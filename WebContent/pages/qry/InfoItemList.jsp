...
function showTree(pid) {
    ...
    // onlyWageItem=true 仅查看本人帐套项目
    xmlHttp.open("GET", "../ajax/InfoItem.jsp?orderByName=true&rightFlag=Y&setId=" + setId, true);
    xmlHttp.send(null);
...
root = tree.root;
    ...
    if(types != null && types.size()>0){
        for(int i=0;i<types.size();i++){
            ...
            if("-1".equals(citem.getItemSuper())){
                String str = "T" + citem.getItemId() + " = tree.add(root,'last','" + citem.getItemName() + "', 'T" + citem.getItemId() + "','','','book_close','','');";
                out.println(str);
            }else{
                String str = "T" + citem.getItemId() + " = tree.add(T" + citem.getItemSuper() +",'last','" + citem.getItemName() + "', 'T" + citem.getItemId() + "','','','book_close','','');";
                out.println(str);
            }
            ...
            if(!"A".equals(setType)){
                ...
                if(alist != null){
                    for(int j=0;j<alist.size();j++){
                        ...
                        String str =s.getSetId() + " = tree.add(" + rootNode + ",'last','" + s.getSetName() + "','" + s.getSetId() + "','"+s.getSet_rsType()+"','','book_close','','');";
                        out.println(str);
        ...
        if("A".equals(setType)){
            ...
            for(int i=0;i<sets.length;i++){
                ...
                String str =s.getSetId() + " = tree.add(" + type + ",'last','" + s.getSetName() + "','" + s.getSetId() + "','"+s.getSet_rsType()+"','','book_close','','');";
                out.println(str);
                str = s.getSetId() + "loading = tree.add(" + s.getSetId() + ",'last','loading...');";
                out.println(str);
                str = s.getSetId() + ".expand(false);";
    ...
    tree.ondblclick = function(srcNode) {
        if (srcNode.getKey() != null && srcNode.getKey().length == 7) {
            parent.doSel(srcNode);
        }
    }
