...
<x:saveState value="#{operation_signInEmailEditbb}" />
<h:form id="form1">
	<h:inputHidden value="#{operation_signInEmailEditbb.pageInit}"></h:inputHidden>
	<h:panelGrid styleClass="td_title" width="100%" border="0"
		cellpadding="0" cellspacing="6" bgcolor="#FFFFFF" columns="2">
		<h:panelGroup>
			<h:graphicImage value="/images/tips.gif" />
			<h:outputText value=" 业务管理-> 设置通知报到邮件格式" />
		</h:panelGroup>
	</h:panelGrid>
	<br>
	...
	<h:panelGrid align="center" width="60%" columns="1">
		...
		<h:panelGrid columns="2" width="80%"
			...
			<h:panelGrid columns="3" width="100%">
				<h:panelGrid>
				<h:outputText value="设置通知报到邮件格式" />
				</h:panelGrid>
				<h:panelGrid align="right">
				<h:selectOneMenu value="#{operation_signInEmailEditbb.param}">
	                    <c:selectItem itemLabel="选择输入参数" itemValue=""></c:selectItem>
	                    <c:selectItem itemLabel="报到人姓名" itemValue="$NAME$"></c:selectItem>
	                    <c:selectItem itemLabel="报到地址" itemValue="$ADDRESS$"></c:selectItem>
	                    <c:selectItem itemLabel="报到时间" itemValue="$DATE$"></c:selectItem>
	                    <c:selectItem itemLabel="系统时间" itemValue="$SYS_DATE$"></c:selectItem>
	            </h:selectOneMenu>
	            </h:panelGrid>
	            <h:commandButton styleClass="button01" value="加入参数" action="#{operation_signInEmailEditbb.interParame}"/>
			</h:panelGrid>
			<h:outputText value="" style="display:none;"/>
			<h:inputTextarea id="desc"
				value="#{operation_signInEmailEditbb.sgContent}" cols="140"
				rows="20"/>
		</h:panelGrid>
		<h:panelGrid columns="2" align="right" cellspacing="2">
			<h:commandButton styleClass="button01" value="保存"
				action="#{operation_signInEmailEditbb.save}"
