...
<script>
   function showResult(personId,staus){
	   window.showModalDialog("/recruit/resumeDetail.jsf?personId="+personId+"&status="+staus, "", "dialogWidth:"+screen.width*0.7+"px; dialogHeight:"+screen.height*0.8+"px; status:0;resizable:yes");
       return true;
   }
...
<h:form id="form1">
    ...
    <x:dataTable value="#{recu_resumeInfoBB.outterResumePersonList}" var="list" align="center" id="dateList1"
        ...
        <h:column>
            ...
            <h:commandButton value="同意面试" styleClass="button01" action="#{recu_resumeInfoBB.agreeIv}" rendered="#{list.status=='0'&&list.firstAuditStatus=='1'}">
            	<x:updateActionListener value="#{list.detailId}" property="#{recu_resumeInfoBB.detailId}"></x:updateActionListener>
       		</h:commandButton>
       		<h:commandButton value="退回" styleClass="button01" action="#{recu_resumeInfoBB.refuseIv}" rendered="#{list.status=='0'&&list.firstAuditStatus=='1'}">
            	<x:updateActionListener value="#{list.detailId}" property="#{recu_resumeInfoBB.detailId}"></x:updateActionListener>
       		</h:commandButton>
