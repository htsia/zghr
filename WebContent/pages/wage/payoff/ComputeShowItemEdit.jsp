...
%>
    ...
    <script type="text/javascript">
        ...
        function forSaveItem() {
            ...
            var allfield = form1.all('form1:allItem');
            count = showfield.length;
            size = showfield.length;
            if (count > 0) {
                revalue = "";
                size=allfield.length;
                for (i = 0; i < size; i++) {
                    revalue += allfield.options[i].value + ",";
                }
                revalue+="|";
                for (i = 0; i < count; i++) {
    ...
    <h:panelGrid columns="3" align="center" width="80%"      styleClass="table03" columnClasses="td_middle,td_middle,td_middle">
        <h:panelGrid columns="1">
            <h:panelGroup>
                ...
                <h:selectBooleanCheckbox value="#{wage_computeBB.orderByName}" onclick="submit()" valueChangeListener="#{wage_computeBB.changeOrder}"></h:selectBooleanCheckbox>
                <h:outputText value="按名称排序"></h:outputText>
            </h:panelGroup>
            <h:selectManyListbox id="allItem" style="width:15em" size="30" >
                <c:selectItems value="#{wage_computeBB.allList}"/>
            </h:selectManyListbox>
        </h:panelGrid>
        ...
        <h:panelGrid columns="1">
            ...
            <h:selectManyListbox id="showItem" style="width:15em" size="30" >
                <c:selectItems value="#{wage_computeBB.showList}"/>
            </h:selectManyListbox>
        </h:panelGrid>    
    </h:panelGrid>

    <c:verbatim><br></c:verbatim>
    <h:inputHidden value="#{wage_computeBB.setId}"></h:inputHidden>
