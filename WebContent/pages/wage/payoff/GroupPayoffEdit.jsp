...
%>
    <script type="text/javascript">
        ...
        function SelectCreateBankFile(){
            ...
            window.showModalDialog(url, null, "dialogWidth:600px; dialogHeight:300px;center:center;resizable:yes;status:no;scroll:yes;");
            return false;
        }
        function OpenRpt(){
            if (document.all("form1:ReportID").value=="-1"){
        ...
        function doDetailPopup() {
            var dateId = form1.all("form1:dateId").value;
            popup = window.open("/wage/payoff/WageDetailView.jsf?dateId=" + dateId, "", "height=600,width=800,toolbar=no,menubar=no,scrollbars=yes,resizable=yes");
            popup.focus();
        }
...
<h:form id="form1">
    <h:inputHidden id="unitId" value="#{wage_flowBB.initGrouppay}"/>
    <h:panelGrid styleClass="td_title" width="100%" border="0" cellpadding="0" cellspacing="6" bgcolor="#FFFFFF" columns="2">
       <h:panelGroup>
         <h:graphicImage value="/images/tips.gif" />
         <h:outputText value="薪酬管理 -> 集团发放"/>
       </h:panelGroup>
    </h:panelGrid>

    <c:verbatim><br></c:verbatim>
    <h:panelGrid columns="2" align="center" width="60%">
        <h:outputText escape="false" value="<strong>发薪单位：</strong>#{wage_flowBB.unitName}"/>
        <h:outputText escape="false" value="<strong>账套：</strong>#{wage_flowBB.setName}"/>
    </h:panelGrid>
    ...
    <h:panelGrid columns="2"  align="center" width="60%" columnClasses="td_form01,td_form02"   styleClass="table03">
        ...
        <h:panelGroup>
            <h:selectOneMenu id="ReportID" style="width:150px">
               <c:selectItems value="#{wage_flowBB.grouppayoffreportList}"/>
            </h:selectOneMenu>
        ...
        <h:panelGroup>
            ...
            <h:commandButton value="生成" styleClass="button01" type="button" onclick="return CreateBankFile();"/>
            <h:commandButton value="选择生成" styleClass="button01" type="button" onclick="return SelectCreateBankFile();"/>
        </h:panelGroup>

        <h:outputText value="报税"/>
        <h:commandButton value="生成报税报表" styleClass="button01" type="button" onclick="return doTaxExport()"/>

        <h:outputText value="相关操作"/>
        <h:panelGroup>
            <h:commandButton value="结束集团发放" action="#{wage_flowBB.endGroupPay}" styleClass="button01"
                             onclick="return confirmSubmitEnd()"/>
            <h:commandButton type="button" value="返回" onclick="window.close()" styleClass="button01" />
        </h:panelGroup>
    </h:panelGrid>
    <c:verbatim><br></c:verbatim>
</h:form>
