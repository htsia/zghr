        ...
        <table align="center" border="0" cellpadding="0" cellspacing="0" width=100% bgcolor="#FFFFFF" >
            <tr>
                <td class=td_title>
                    ...
                    <h:outputText value=" 薪酬管理 ->  薪资项目录入与计算"/>
      <f:verbatim>
                </td>
    ...
    <f:verbatim>
          ...
          <tr>
                <td colspan=2 valign="bottom" align="center">
                    ...
                    <h:inputTextarea rows="6" cols="130" value="#{wage_dateBB.wageMemo}"/>
                    <h:commandButton value="保存记事" styleClass="button01" action="#{wage_dateBB.saveWageMemo}"></h:commandButton>
                    <h:commandButton value="表格对应" styleClass="button01" type="button" onclick="doRptWageSet();"></h:commandButton>
    <f:verbatim>
                   <br>
                </td>
            </tr>
...
<f:verbatim>
            ...
            <td align="right">
                ...
                <h:commandButton value="开始新发放" action="#{wage_dateBB.save}" styleClass="button01"  rendered="#{wage_dateBB.canOperNew}" onclick="return forAddDate()"/>
                <h:commandButton value="设置财务处理项目" action="#{wage_dateBB.saveFinanceItem}" styleClass="button01" onclick="return forFinanceDate()" rendered="false"/>
                <h:commandButton type="button" styleClass="button01" value="人员增减变动" onclick="queryPersonChange();"></h:commandButton>
                <h:commandButton type="button" styleClass="button01" value="薪资部门变动" onclick="queryDeptChange();"></h:commandButton>
                <h:commandButton type="button" styleClass="button01" value="常量变化" onclick="queryConstChange();"></h:commandButton>
<f:verbatim>
            </td></tr>
        </table>
      </td></tr>
     ...
     <tr><td>
        ...
        <h:dataTable value="#{wage_dateBB.dateList}" var="list" align="center"  id="dateList"
            ...
            <h:column>
                <c:facet name="header"><h:outputText value="允许录入"/></c:facet>
                <h:outputText value="#{list.forbidDes}"/>
            </h:column>

            <h:column>
                <c:facet name="header"><h:outputText value="状态"/></c:facet>
                <h:outputText escape="false" value="<font color=red>#{list.status}</font>"/>
            </h:column>
            ...
            <h:column rendered="#{wage_dateBB.name0659!=''}">
                <c:facet name="header" ><h:outputText value="#{wage_dateBB.name0659}"/></c:facet>
            ...
            <h:column rendered="#{wage_dateBB.groupPayoff}">
                <c:facet name="header"><h:outputText value="集团发放状态"/></c:facet>
            ...
            <h:column>
                ...
                <h:panelGrid columns="5" align="center" cellspacing="2">
                    <h:commandButton value="删除发放" styleClass="button01" rendered="#{list.operDel}"
                    ...
                    <h:commandButton value="打开发放" action="#{wage_computeBB.queryPersonBySetId}"
                    ...
                    <h:commandButton value="允许录入" styleClass="button01" rendered="#{list.operCompute && list.forbid=='1'}"
                                     action="#{wage_dateBB.UnLock}" >
                        <x:updateActionListener property="#{wage_dateBB.dateId}" value="#{list.dateId}"/>
                    </h:commandButton>

                    <h:commandButton value="录入数据" styleClass="button01" rendered="#{list.operInput}"
                                     action="#{wage_inputManagerBB.queryPersonBySetId}">
                        <x:updateActionListener property="#{wage_inputManagerBB.dateId}" value="#{list.dateId}"/>
                        <x:updateActionListener property="#{wage_inputManagerBB.payoffDate}" value="#{list.date}"/>
                        <x:updateActionListener property="#{wage_inputManagerBB.setId}" value="#{wage_dateBB.setId}"/>
                        <x:updateActionListener property="#{wage_inputManagerBB.unitId}" value="#{wage_dateBB.unitId}"/>
                        <x:updateActionListener property="#{wage_inputManagerBB.setName}" value="#{wage_dateBB.setName}"/>
                        <x:updateActionListener property="#{wage_inputManagerBB.unitName}" value="#{wage_dateBB.unitName}"/>
                        <x:updateActionListener property="#{wage_inputManagerBB.operCompute}" value="#{list.operCompute}"/>
                        <x:updateActionListener property="#{wage_inputManagerBB.operAuditInput}" value="false"/>
                    </h:commandButton>
                   

                    <h:commandButton value="录入明细" styleClass="button01" rendered="#{list.operViewInput}"
                                     type="button" onclick="doInputDetailPopup('#{list.dateId}')"/>
