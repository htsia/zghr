    ...
    <h:form id="form1">
        ...
        <h:panelGrid align="right" styleClass="locked_top">
         <h:panelGroup>
             ...
             <h:commandButton value="首页" action="#{wage_commonAdjustBB.first}" styleClass="button01"></h:commandButton>
             <h:commandButton value="上页" action="#{wage_commonAdjustBB.pre}" styleClass="button01"></h:commandButton>
             <h:commandButton value="下页" action="#{wage_commonAdjustBB.next}" styleClass="button01"></h:commandButton>
             <h:commandButton value="尾页" action="#{wage_commonAdjustBB.last}" styleClass="button01"></h:commandButton>
             <h:commandButton value="流程说明" onclick="return showWorkDefine('<%=WFTypeBO.WAGE_ADJUST_AUDIT%>');" styleClass="button01" rendered="#{wage_commonAdjustBB.followFlow}"></h:commandButton>
             <h:commandButton value="设置" onclick="return setRule('');" type="button"  styleClass="button01"></h:commandButton>
             <h:commandButton value="增加" onclick="return fPopUpBackPerTreeDlg('');" action="#{wage_commonAdjustBB.selPerson}"  styleClass="button01"></h:commandButton>
         </h:panelGroup>
     </h:panelGrid>
        <c:verbatim>
            </td></tr>
        ...
        <h:column >
            <c:facet name="header" ><h:outputText value="操作" style="width:120px"/></c:facet>
                <h:commandButton value="拟定" styleClass="button01"  action="#{wage_commonAdjustBB.doQuery}" onclick="return setItem('#{list.adjustbo.itemID}');"  rendered="#{list.adjustbo.status=='0'}"/>
                ...
                <h:commandButton value="生效" styleClass="button01" onclick="return doOK('#{list.adjustbo.itemID}');" action="#{wage_commonAdjustBB.doQuery}" rendered="#{list.adjustbo.status=='2' || (list.adjustbo.status=='0' && sys_commonInfoBB.wage_adjust_mode=='1')}"/>
