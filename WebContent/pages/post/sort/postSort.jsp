...
<x:saveState value="#{postSortBB}"/>
<h:form id="form1">
    <h:inputHidden value="#{postSortBB.pageInit}"/>
<c:verbatim>
    <table align=center width="80%" border="0" cellpadding="2" cellspacing="0">
</c:verbatim>
        <%
        	List<PostBO> list = (List<PostBO>)session.getAttribute("postList");
...
<c:verbatim>
            ...
            <td class="td_middle" width="90%">
                <select name="showItem" size="28" multiple id="showItem" style="width:100%;">
                    <%
                        for (PostBO p : list) {
                    %>
                    <option value="<%=p.getPostId()%>"><%=p.getName()%></option>
                    <%
                        }
                    %>
                </select>
        ...
        <tr>
            ...
            <td class="td_top"   align="right">
              ...
              <h:commandButton value="保存" styleClass="button01" onclick="return forSave();" action="#{postSortBB.saveSort}"/>
<c:verbatim>
            </td>
        </tr>
</c:verbatim>
...
<c:verbatim>
        <tr>
            <br>
            <td  colspan="2" align="center">无岗位！</td>
        </tr>
</c:verbatim>
        <%
            }
