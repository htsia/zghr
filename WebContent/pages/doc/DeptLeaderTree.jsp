...
//展示树要调用的事件
function showTree(pid, rightFlag, rootId,leaderFlag) {
    flag = false;
    ...
    xmlHttp.open("GET", "/pages/ajax/DeptLeaderTree.jsp?leaderFlag="+leaderFlag+"&superId=" + superId + "&rightFlag=1&rootId=" + rootId + "&filterSQL=<%=CommonFuns.filterNull(request.getParameter("filterSQL"))%>" , true);
    xmlHttp.send(null);
}
...
function parseResults() {
    ...
    if (tree.nodes['n' + targetTreeId] != null) {
        ...
        tree.nodes['n' + targetTreeId].expand(true);//最末一层展开
    }
    try{
...
<script type="text/javascript">
    ...
    showTree("-1", '<%=CommonFuns.filterNull(request.getParameter("rightFlag"))%>', '<%=CommonFuns.filterNull(request.getParameter("rootId"))%>',1);//树的根节点
    //定义树的节点的扩展方法
    tree.onexpand = function(srcNode) {
        if (srcNode.first.label.innerText == "loading...") {
            if (flag) {
            	//alert("查询"+targetTreeId+">>>>"+srcNode.key2);
            	showTree(srcNode.key2, '', '',0);
            } else
