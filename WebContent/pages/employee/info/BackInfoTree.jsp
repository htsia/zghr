...
for(int i=0;i<types.size();i++){
    ...
    if(!citem.getTreeId().startsWith(typeId))
        continue;
      if (!citem.getItemId().substring(0,8).equals("20010108") && !citem.getItemId().equals("2001010000"))
            continue;
...
//画指标集
String backSets = Constants.EMP_BACKPERSON_SETS;
if(list != null && list.size() >0){
for(int i=0;i<list.size();i++){
   InfoSetBO set = (InfoSetBO)list.get(i);
    if(backSets.indexOf(set.getSetId()) == -1){
        continue;
    }
