...
<script type="text/javascript">
    function ModifyNo(id){
        var ret=window.showModalDialog("/employee/chg/ModifyLetterNo.jsf?letterType=cadres&ID="+id, null, "dialogWidth:300px; dialogHeight:200px;center:center;resizable:no;status:no;");
        return false;
    }

    function openDetail(id){
        var ret=window.showModalDialog("/employee/chg/CadresLetterDetailList.jsf?LetterNO="+id, null, "dialogWidth:"+screen.width*0.85+"px; dialogHeight:"+screen.height*0.85+"px;center:center;resizable:no;status:no;");
        return false;
    }
    function doOpenCadresLetter() {
         var ret=window.showModalDialog("/employee/chg/OpenCadresLetter.jsf?Init=true", null, "dialogWidth:"+screen.width*0.85+"px; dialogHeight:"+screen.height*0.85+"px;center:center;resizable:no;status:no;");
         return true;
    }
    function OpenRpt(id){
        ...
        var url="/pages/report/ServerReportShow.jsp?RptCode="+document.all("form1:ReportID").value+"&ID="+id;
        window.open(url);
        return false;
    }
    function OpenRptOrFlow(id){
        ...
        var url="/pages/report/RollORFlowReportShow.jsp?RptCode="+document.all("form1:ReportID").value+"&ID="+id+"&Title=CD_"+id;
        window.open(url);
        return false;
    }
...
<h:form id="form1">
    <h:inputHidden value="#{emp_letterBB.cadresPage}"></h:inputHidden>
    <c:verbatim>
            ...
            <table><tr>
                <td>
                    ...
                    <h:selectOneMenu id="ReportID" style="width:190px">
                        <c:selectItems value="#{emp_letterBB.cadresReport}"/>
                    </h:selectOneMenu>
<c:verbatim>
                </td>
...
<c:verbatim>
                ...
                <td>
                    ...
                    <h:commandButton value="首页" action="#{emp_letterBB.cadresfirst}" styleClass="button01"></h:commandButton>
                    <h:commandButton value="上页" action="#{emp_letterBB.cadrespre}" styleClass="button01"></h:commandButton>
                    <h:commandButton value="下页" action="#{emp_letterBB.cadresnext}" styleClass="button01"></h:commandButton>
                    <h:commandButton value="尾页" action="#{emp_letterBB.cadreslast}" styleClass="button01"></h:commandButton>
                    <h:outputText value="     "></h:outputText>
                    <h:commandButton value="新开干部调转介绍信" styleClass="button01" onclick="doOpenCadresLetter();"></h:commandButton>
...
<c:verbatim>
        ...
        <tr><td>
         <div style='width:100%;height:100%;overflow:auto' id='datatable'>
            ...
            <h:dataTable value="#{emp_letterBB.letterList}" var="list" width="100%"
