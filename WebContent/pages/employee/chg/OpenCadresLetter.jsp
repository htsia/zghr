...
<h:form id="form1">
	<h:inputHidden id="initWorkLetter"
		value="#{emp_letterBB.initCadresLetter}" />
	<h:inputHidden id="ids" value="#{emp_letterBB.ids}"></h:inputHidden>
	<c:verbatim>
		<table width="100%" class="td_title">
			<tr>
				<td height="25px" width="50%"></c:verbatim> <h:graphicImage
						value="/images/tips.gif" /> <f:verbatim>
						<%=LanguageSupport.getResource("RYGL-2160",
							"新开干部调转介绍信")%>
					</f:verbatim> <c:verbatim>
				</td>
				<td height="25px" width="50%" align="right"></c:verbatim> <h:commandButton
						styleClass="button01" value="确认" onclick="return doCheck();"
						action="#{emp_letterBB.openCadLetter}"></h:commandButton> <h:commandButton
	...
	<h:panelGrid columns="1" width="98%" cellspacing="2" align="center">
		<h:panelGroup>
			<f:verbatim>
				<%=LanguageSupport.getResource("RYGL-2161",
									"介绍信编号")%>:
	        </f:verbatim>
			<h:inputText value="#{emp_letterBB.cdbo.letterNO}" />
			<h:outputText value="    "></h:outputText>
			<f:verbatim>
				<%=LanguageSupport.getResource("RYGL-2207",
									"选择调往单位")%>:
	        </f:verbatim>
			...
			<h:commandButton value="查询" styleClass="button01"
				action="#{emp_letterBB.getCadPersonBySupeID}"></h:commandButton>
		</h:panelGroup>
