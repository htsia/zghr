...
<h:form id="form1">
	<h:inputHidden id="initEdit" value="#{empApporDisMgrBB.initOrgEdit}"></h:inputHidden>
	<h:inputHidden value="#{empApporDisMgrBB.copySet}" id="copySet" />
	<h:inputHidden value="#{empApporDisMgrBB.unitId}" id="unitId" />
	<h:inputHidden value="#{empApporDisMgrBB.setId}" id="setId" />
		...
		<h:panelGroup>
			...
			<f:verbatim>
				<%=LanguageSupport.getResource("RYGL-0300",
									"人员管理 ")%>->
			        <%=LanguageSupport.getResource("RYGL-2045",
									"机构任免设置")%>
			</f:verbatim>
		</h:panelGroup>
	</h:panelGrid>
	...
	<h:panelGrid columns="1" width="98%" align="left">
		<h:panelGrid columns="2" columnClasses="td_form01,td_form02"
			...
			<h:inputText value="#{empApporDisMgrBB.orgappordischangebo.orgName}"
			...
			<h:panelGrid width="98%" align="center">
				<h:panelGrid width="98%" align="right">
					<h:inputHidden id="infoItem" value="#{empApporDisMgrBB.infoItem}"></h:inputHidden>
					<h:panelGroup>
						<h:commandButton value="增加" action="#{empApporDisMgrBB.addfield}"
							onclick="return selectInfoItem();" styleClass="button01"></h:commandButton>
					</h:panelGroup>
				</h:panelGrid>

				<h:dataTable value="#{empApporDisMgrBB.baseFields}" var="list"
					...
					<h:column>
						...
						<h:commandButton styleClass="button01" value="删除"
							action="#{empApporDisMgrBB.delfield}" onclick="checkform()">
							<x:updateActionListener property="#{empApporDisMgrBB.infoItem}"
								value="#{list.itemId}" />
						</h:commandButton>
						<h:commandButton styleClass="button01" value="上移"
							action="#{empApporDisMgrBB.movePre}" onclick="checkform()">
							<x:updateActionListener property="#{empApporDisMgrBB.infoItem}"
								value="#{list.itemId}" />
						</h:commandButton>
						<h:commandButton styleClass="button01" value="下移"
							action="#{empApporDisMgrBB.moveNext}" onclick="checkform()">
							<x:updateActionListener property="#{empApporDisMgrBB.infoItem}"
								value="#{list.itemId}" />
						</h:commandButton>
					</h:column>
		...
		<h:panelGrid align="right">
			<h:commandButton value="保存"
				action="#{empApporDisMgrBB.saveOrgPostChangeBO}"
