...
function clickSubmit(){
  ...
  <h:form id="form1">
<h:inputHidden id="initEdit" value="#{emptitledeptAduitmgrBB.initInputScore}"></h:inputHidden>
<h:inputHidden id="hiScore" value="#{emptitledeptAduitmgrBB.hiScore}"></h:inputHidden>
<h:inputHidden id="lowScore" value="#{emptitledeptAduitmgrBB.lowScore}"></h:inputHidden>
<f:verbatim>
...
<table width=95%><tr><td align="center">
            ...
            <h:inputText id="score" value="#{emptitledeptAduitmgrBB.scoreBo.score}" style="display:none"></h:inputText>
            <h:outputText value="  "></h:outputText>
            <h:commandButton style="display:none" id="submit" value="打分" onclick="return saveScore();" action="#{emptitledeptAduitmgrBB.saveScore}"></h:commandButton>
<f:verbatim>
</td></tr></table>
<br>
<%
...
List deptItemList=(List)session.getAttribute("deptItemList");
...
if(deptItemList!=null&&deptItemList.size()>0){
		for(int i=0;i<deptItemList.size();i++){
			EmpTitleApplyScoreBO score=(EmpTitleApplyScoreBO)deptItemList.get(i);
			EmpTitleTempItemBO temItem=setUcc.findEmpTitleTempItemBO(score.getTempItemId());
			if(score.getScoreId().equals(scoreId)){

%>
	<td align="center" width="16%" class=activeTab><a href="deptAudit_inputScore.jsf?scoreId=<%=score.getScoreId()%>"><%=temItem.getItemName() %></a></td>
 <%
			}else{
				%>
	<td align="center" width="16%" class=tab><a href="deptAudit_inputScore.jsf?scoreId=<%=score.getScoreId()%>"><%=temItem.getItemName() %></a></td>
				<%
			}
	}
}
