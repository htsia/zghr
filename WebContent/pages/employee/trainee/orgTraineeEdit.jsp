...
function checkform() {
 if(document.all("form1:probation").value==""||document.all("form1:probation").value==null){
	 alert("试用期不能为空！");
	 return false;
 }else{
...
<h:form id="form1">
	<h:inputHidden id="initEdit" value="#{empTraineeMgrBB.initOrgEdit}"></h:inputHidden>
	<h:inputHidden value="#{empTraineeMgrBB.copySet}" id="copySet" />
	<h:inputHidden value="#{empTraineeMgrBB.unitId}" id="unitId" />
	<h:inputHidden value="#{empTraineeMgrBB.setId}" id="setId" />
	...
	<h:panelGrid columns="1" width="98%" align="left">
		<h:panelGrid columns="2" columnClasses="td_form01,td_form02"
			...
			<h:inputText value="#{empTraineeMgrBB.orgchangebo.orgName}"
				readonly="true"></h:inputText>
			<f:verbatim>
				<%=LanguageSupport.getResource("RYGL-2142",
									"试用期")%>
			</f:verbatim>
			<h:inputText id="probation" value="#{empTraineeMgrBB.probationTime}"></h:inputText>
			<f:verbatim>
			...
			<h:panelGrid width="98%" align="center">
				<h:panelGrid width="98%" align="right">
					<h:inputHidden id="infoItem" value="#{empTraineeMgrBB.infoItem}"></h:inputHidden>
					<h:panelGroup>
						<h:commandButton value="增加" action="#{empTraineeMgrBB.addfield}"
							onclick="return selectInfoItem();" styleClass="button01"></h:commandButton>
					</h:panelGroup>
				</h:panelGrid>

				<h:dataTable value="#{empTraineeMgrBB.baseFields}" var="list"
					...
					<h:column>
						...
						<h:commandButton styleClass="button01" value="删除"
							action="#{empTraineeMgrBB.delfield}" onclick="checkform()">
							<x:updateActionListener property="#{empTraineeMgrBB.infoItem}"
								value="#{list.itemId}" />
						</h:commandButton>
						<h:commandButton styleClass="button01" value="上移"
							action="#{empTraineeMgrBB.movePre}" onclick="checkform()">
							<x:updateActionListener property="#{empTraineeMgrBB.infoItem}"
								value="#{list.itemId}" />
						</h:commandButton>
						<h:commandButton styleClass="button01" value="下移"
							action="#{empTraineeMgrBB.moveNext}" onclick="checkform()">
							<x:updateActionListener property="#{empTraineeMgrBB.infoItem}"
								value="#{list.itemId}" />
						</h:commandButton>
					</h:column>
		...
		<h:panelGrid align="right">
			<h:commandButton value="保存"
				action="#{empTraineeMgrBB.saveOrgProciseBO}" styleClass="button01"
